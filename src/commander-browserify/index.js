...
class Command extends EventEmitter {
  ...
  command(nameAndArgs, actionOptsOrExecDesc, execOpts) {
    ...
    if (desc) {
      throw new Error("don't support Stand-alone executable (sub)commands");
    }
  ...
  _exit(exitCode, code, message) {
    ...
    // exit
    throw new Error(this._exitCode);
  }
  ...
  _checkForOptionNameClash(option) {
    if (
      ...
      this._storeOptionsAsPropertiesCalled
    ) {
...
Read more on https://git.io/JJc0W`);
   ...
   * @param {string} [parseOptions.from] - where the args are from: 'user'
   * @return {Command} `this` command for chaining
   * @api public
   */
  ...
  parse(argv, parseOptions) {
    ...
    if (argv === undefined) {
      throw new Error('argv not found');
    }
  ...
  _parseCommand(operands, unknown) {
    ...
    if (operands && this._findCommand(operands[0])) {
      ...
      this._lazyHasImplicitHelpCommand() &&
  ...
  optionHelp() {
    ...
    function padOptionDetails(flags, description) {
      return (
        pad(flags, width) +
        '  ' +
        optionalWrap(description, descriptionWidth, width + 2)
      );
    }
    ...
    const showShortHelpFlag =
      this._hasHelpOption &&
      this._helpShortFlag &&
      !this._findOption(this._helpShortFlag);
    const showLongHelpFlag =
      this._hasHelpOption && !this._findOption(this._helpLongFlag);
